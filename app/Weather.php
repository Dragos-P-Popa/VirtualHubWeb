...
class Weather {
	...
	private static $weather_condition = [ "Good" => 0, "Moderate" => 0, "Bad" => 0 ];
	...
	static function ForAirport( $icao ) {
		...
		if ( $array["data"]["@attributes"]["num_results"] == 0 ) {
			...
			if (isset($array["data"]["METAR"][0])) {
				return self::parseWeather( $array["data"]["METAR"][0] );
			} else {
				return self::parseWeather( $array["data"]["METAR"]);
			}
		}
	}
	...
	static private function parseWeather( $response ) {
		...
		$overall_condition = array_search( max( self::$weather_condition ), self::$weather_condition );
